...
from .common import MZNetworkOutput, RepresentationNetwork, PredictionNetwork, SimpleDecoder
...
class MuZeroModel(nn.Module):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.decoder = SimpleDecoder(
            num_channels, # in channels
            observation_shape[0]  # out channels
        )
    ...
    def initial_inference(self, obs: torch.Tensor) -> MZNetworkOutput:
        ...
        #daev
        reconstruction = self._reconstruction(latent_state)
    ...
    def recurrent_inference(self, latent_state: torch.Tensor, action: torch.Tensor) -> MZNetworkOutput:
        ...
        reconstruction = self._reconstruction(latent_state)
        return MZNetworkOutput(value, reward, policy_logits, next_latent_state, reconstruction)
    ...
    def _reconstruction(self, latent_state: torch.Tensor) -> torch.Tensor:
        """
        Overview:
            Use the decoder network to reconstruct the observations from the latent state.
        Arguments:
            - obs (:obj:`torch.Tensor`): The encoding latent state of input state.
        Returns:
            - reconstruction (:obj:`torch.Tensor`): The decodedencoding latent state of input state.
        Shapes:
            - latent_state (:obj:`torch.Tensor`): :math:`(B, H_, W_)`, where B is batch_size, H_ is the height of \
                latent state, W_ is the width of latent state.
            - reconstruction (:obj:`torch.Tensor`): :math:`(B, num_channel, obs_shape[1], obs_shape[2])`, where B is batch_size.
        """
